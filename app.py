...
def arrow_safe(df: pd.DataFrame) -> pd.DataFrame:
    """These trackers routinely mix types within one 'object' column (a
    date in most rows, a stray string like 'na' or a number in others -
    e.g. the ZeCom 'EOL Contract END DATES' column). Streamlit's Arrow
    serializer throws on that, so stringify object columns for *display
    only* - the real dataframes used for matching keep their original
    values untouched."""
    safe = df.copy()
...
def smart_upload(
    ...
    with st.expander(f"⚙️ {label} — header row & column mapping", expanded=False):
        ...
        st.dataframe(arrow_safe(df.head(5)), width="stretch")
...
# --------------------------------------------------------------------------
    ...
    if "result_df" in st.session_state:
        ...
        st.dataframe(arrow_safe(filtered), width="stretch", height=500)
