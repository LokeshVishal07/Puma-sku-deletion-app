...
ROLE_KEYWORDS = {
    ...
    "seller_sku": [
        "sellersku", "seller sku", "seller_sku",
    ],
    "generic_sku": ["sku"],
    "article_no": [
    ...
    "status": ["status"],
    # The platform's own listing/product identifier - needed alongside the
    # Seller SKU to actually go delete/deactivate a listing in each
    # marketplace's seller center (Lazada Product ID, Shopee Product ID,
    # Zalora ShopSku, TikTok Product ID all serve this role).
    "platform_product_id": [
        "product id", "productid", "shop sku", "shopsku", "pid",
    ],
}

# "sku" alone is too generic to trust via substring match - it would also
# match unrelated columns like Lazada's "sku.skuId" (an internal listing
# id, not a barcode) or Shopee's "Parent SKU" (an inconsistent grouping
# id). It's only ever accepted when a column's name is *exactly* "sku"
# (e.g. the MY inventory file's barcode column is literally named "Sku"),
# and only after every more specific keyword above has already failed.
EXACT_ONLY_FALLBACK_KEYWORDS = {
    "ean": ["sku"],
    "seller_sku": ["sku"],
}
...
def guess_column(columns: list[str], role: str) -> str | None:
    ...
    # Pass 1: exact match, keyword order = priority. This matters a lot for
    # e.g. Shopee's plain "SKU" column (the actual barcode/seller sku)
    # vs. its "Parent SKU" column (an inconsistent, often-blank grouping
    # id) - both contain the substring "sku", but only "SKU" should win.
    for kw in keywords:
        for col, low in lower_cols.items():
            if low == kw:
                return col
    # Pass 2: substring match, same keyword priority order.
    for kw in keywords:
        for col, low in lower_cols.items():
            if kw in low:
                return col
    # Pass 3: generic "sku" fallback - exact match only (see comment on
    # EXACT_ONLY_FALLBACK_KEYWORDS above for why this can't be a substring
    # match).
    for kw in EXACT_ONLY_FALLBACK_KEYWORDS.get(role, []):
        for col, low in lower_cols.items():
            if low == kw:
                return col
    return None
